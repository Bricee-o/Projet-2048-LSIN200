...
def supprime():
    for i in range(len(case_graphique)):
            canvas.delete(case_graphique[i])
...
def tablo():
    ...
    for i in range(len(table)):
        for j in range(len(table[0])):
            color = "gray80"
            rect = canvas.create_rectangle((i*largeur_case, j*hauteur_case),((i+1)*largeur_case, (j+1)*hauteur_case), fill=color)
            if table[i][j]!=0:
                t = canvas.create_text(i*largeur_case+62, j*hauteur_case+62, text=table[i][j], font=("helvetica", "30"))
                case_graphique.append(t)
            case_graphique.append(rect)
    canvas.update()


def cheat():
    global table
    table = [[2, 120, 8, 16],[32, 4, 2, 128],[64, 256, 512, 4],[256, 8, 32, 4]]
    tablo()
    score()
...
def inp(event):
    ...
    score()   
    tablo()     
    key = event.keysym               #La fonction appear quand le clavier est utilisé, je sais pas si on peut limiter le truc à 4 bouton par contre
    ...
    elif key == 'Up' and 'U' in poss:    
        game('U')
        a = True

    elif key == 'Down' and 'D' in poss:    
        game('D')
        a = True
